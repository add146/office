...
class Projects_model extends CI_Model
{
    ...
    function get_projects_list()
    {
        ...
        foreach ($results as $result) {
            ...
            if ($result['client_id']) {
                $project_client = $this->ion_auth->user($result['client_id'])->row();
                if ($project_client) {
                    $project_client->company = company_details('company_name', $result['client_id']);
                    $tempRow['project_client'] = $project_client;
                } else {
                    $tempRow['project_client'] = null;
                }
            } else {
                $tempRow['project_client'] = null;
            }
    ...
    function get_projects($user_id = '', $project_id = '', $limit = '', $start = '', $filter_type = '', $filter = '')
    {
        ...
        foreach ($projects as $key => $project) {
            ...
            if ($project['client_id']) {
                $client_user = $this->ion_auth->user($project['client_id'])->row();
                if ($client_user) {
                    $client_user->company = company_details('company_name', $project['client_id']);
                    $temp[$key]['project_client'] = $client_user;
                } else {
