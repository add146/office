...
<body>
          ...
          <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
              ...
              <div class="breadcrumb-item">
                <a
                  href="<?= base_url('invoices') ?>"><?= $this->lang->line('invoices') ? $this->lang->line('invoices') : 'Invoices' ?></a>
              </div>
              <div class="breadcrumb-item">
            ...
            <div class="row mb-4">
              <div class="col-lg-12">
                <?php if ($this->ion_auth->in_group(3) && $invoice[0]['status'] != 1 && ((get_stripe_secret_key() && get_stripe_publishable_key() || get_razorpay_key_id() || get_offline_bank_transfer()))) { ?>
                  <a href="#" class="btn btn-primary btn-icon icon-left payment-button"
                    data-amount="<?= htmlspecialchars($final_total) ?>"
                    data-id="<?= htmlspecialchars($invoice[0]['id']) ?>"><i class="fas fa-credit-card"></i>
                    <?= $this->lang->line('process_payment') ? $this->lang->line('process_payment') : 'Process Payment' ?></a>
                <?php } ?>
                <a href="<?= base_url('invoices/generate-pdf/' . $invoice[0]['id']) ?>"
                  class="btn btn-danger btn-icon icon-left" target="_blank"><i class="fas fa-print"></i>
                  <?= $this->lang->line('print') ? $this->lang->line('print') : 'Print' ?></a>
              </div>
            </div>
                    ...
                    <div class="row">
                      <div class="col-md-6">
                        <address>
                          <h5 class="text-primary text-uppercase">
                            <?= $invoice_from->company_name ? htmlspecialchars($invoice_from->company_name) : htmlspecialchars($invoice[0]['from_user']) ?>
                          </h5>
                          <?= $invoice_from->address ? htmlspecialchars($invoice_from->address) : '' ?><br>
                          <?= $invoice_from->city ? htmlspecialchars($invoice_from->city) : '' ?>,
                          <?= $invoice_from->state ? htmlspecialchars($invoice_from->state) : '' ?><br>
                          <?= $invoice_from->country ? htmlspecialchars($invoice_from->country) : '' ?>
                          <?= $invoice_from->zip_code ? htmlspecialchars($invoice_from->zip_code) : '' ?><br>
                        </address>
                      </div>
                      <div class="col-md-6 text-md-right">
                        <address>
                          ...
                          <?= $invoice_to->city ? htmlspecialchars($invoice_to->city) : '' ?>,
                          <?= $invoice_to->state ? htmlspecialchars($invoice_to->state) : '' ?><br>
                          <?= $invoice_to->country ? htmlspecialchars($invoice_to->country) : '' ?>
                          <?= $invoice_to->zip_code ? htmlspecialchars($invoice_to->zip_code) : '' ?><br>
                        </address>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-md-6">
                        <address>
                          ...
                          <?= htmlspecialchars(format_date($invoice[0]['due_date'], system_date_format())) ?><br>
                      ...
                      <table class="table table-striped table-md">
                        <tr>
                          ...
                          <th class="text-right">
                            ...
                            (<?= htmlspecialchars(get_currency('currency_symbol')) ?>)
                          </th>
                        </tr>
                        ...
                        foreach ($items as $key => $item) {
                          ...
                          <tr>
                            ...
                            <td class="text-right"><?= format_currency($item['budget'], false) ?></td>
                          </tr>
                        <?php } ?>
                        ...
                        <?php foreach ($products as $key => $item) {
                          ...
                          <tr>
                            ...
                            <td class="text-right"><?= format_currency($item['price'], false) ?></td>
                          </tr>
                        <?php } ?>
                    ...
                    <div class="row mt-4">
                      <div class="col-lg-7">
                        <?php if ($invoice[0]['note']) { ?>
                          ...
                          <?= htmlspecialchars($invoice[0]['note']) ?>
                        <?php } ?>
                      </div>
                      <div class="col-lg-5 text-right">
                        <div class="invoice-detail-item">
                          ...
                          <div class="invoice-detail-value"><?= format_currency($invoice[0]['amount']) ?></div>
                        </div>
                        <?php if ($taxes) {
                          foreach ($taxes as $tax) { ?>
                            <div class="invoice-detail-item">
                              <div class="invoice-detail-name"><?= htmlspecialchars($tax['title']) ?>
                                (<?= htmlspecialchars($tax['tax']) ?>%)</div>
                              <div class="invoice-detail-value"><?= format_currency($tax['tax_amount']) ?></div>
                            </div>
                          <?php }
                        } ?>
                        <hr class="mt-2 mb-2">
                        <div class="invoice-detail-item">
                          ...
                          <div class="invoice-detail-value invoice-detail-value-lg"><?= format_currency($final_total) ?>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="col-12">
                      <div class="alert alert-light border">
                        <h6 class="text-primary mb-2">
                          ...
                          /
                          <?= $this->lang->line('custom_payment') ? $this->lang->line('custom_payment') : 'Custom Payment' ?>
                        </h6>
                ...
                <?php if (get_offline_bank_transfer()) { ?>
                  ...
                  <div id="accordion" class="col-md-8 paymet-box mx-auto">
                    <div class="accordion mb-0">
                      <div class="accordion-header text-center" role="button" data-toggle="collapse"
                        ...
                        <form action="<?= base_url('invoices/order-completed/') ?>" method="POST" id="bank-transfer-form">
                          <div class="card-footer bg-whitesmoke">
                            <div class="form-group">
                              <label><?= $this->lang->line('upload_receipt') ? htmlspecialchars($this->lang->line('upload_receipt')) : 'Upload Receipt' ?>
                                <i class="fas fa-question-circle" data-toggle="tooltip" data-placement="right"
                                  title="<?= $this->lang->line('supported_formats') ? htmlspecialchars($this->lang->line('supported_formats')) : 'Supported Formats: jpg, jpeg, png' ?>"
                                  data-original-title="<?= $this->lang->line('supported_formats') ? htmlspecialchars($this->lang->line('supported_formats')) : 'Supported Formats: jpg, jpeg, png' ?>"></i>
                              </label>
                            ...
                            <button
                              class="btn btn-primary savebtn"><?= $this->lang->line('upload_and_send_for_confirmation') ? htmlspecialchars($this->lang->line('upload_and_send_for_confirmation')) : 'Upload and Send for Confirmation' ?></button>
                          </div>
  ...
  <?php if (get_payment_paypal()) { ?>
    <script
      src="https://www.paypal.com/sdk/js?client-id=<?= get_payment_paypal() ?>&currency=<?= get_currency('currency_code') ?>"></script>
  <?php } ?>
