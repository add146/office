	...
	public function edit()
	{
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				// Only validate folder name (language) contains valid characters for directory
				if (preg_match('/[^A-Za-z]/', $new_lang)) {
					$this->data['error'] = true;
					$this->data['message'] = $this->lang->line('only_english_characters_allowed') ? $this->lang->line('only_english_characters_allowed') : "Language name must be English letters only (for folder name).";
					echo json_encode($this->data);
					return false;
				}
